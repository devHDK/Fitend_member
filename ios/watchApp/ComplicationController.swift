...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getComplicationDescriptors(handler: @escaping ([CLKComplicationDescriptor]) -> Void) {
        let descriptors = [
            CLKComplicationDescriptor(identifier: "complication", displayName: "FITEND", supportedFamilies: [.circularSmall, .modularSmall, .utilitarianSmall, .utilitarianLarge, .extraLarge, .graphicCorner, .graphicCircular, .graphicExtraLarge, .modularLarge])
...
//                    fatalError("Graphic Extra Large template is only available on watchOS 7.")
        ...
        private func createModularLargeTemplate(forDate date: Date) -> CLKComplicationTemplate {
            // Create the data providers.
            let titleTextProvider = CLKSimpleTextProvider(text: "FITEND", shortText: "fitend")
            titleTextProvider.tintColor = .init(red: 0.364, green: 0.356, blue: 0.894, alpha: 100)

            let textLine1 = CLKSimpleTextProvider(text: "FITEND")
        ...
        private func createUtilitarianLargeTemplate(forDate date: Date) -> CLKComplicationTemplate {
            // Create the data providers.
            let solText = CLKSimpleTextProvider(text: "FITEND")
